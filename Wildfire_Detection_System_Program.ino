...
void PublishData() {
  ...
  postData = "{\"alert\": " + String(photoAlarm) + ",\"env\": {\"humidity\": " + String(humi) + ",\"ionization\": " + ionSensor + ",\"photoelectric\": " + photoSensor + ",\"temperature\": " + String(temp) + "},\"GPS\": {\"latitude\": " + String(mainLati, 6) + ",\"longitude\": " + String(mainLongi, 6) + "},\"GSM\": {\"latitude\": " + String(gsmLati, 6) + ",\"longitude\": " + String(gsmLongi, 6) + "},\"name\": \"" + DeviceName + "\",\"video\": \"" + SubName(videoName) + "\",\"wind\": {\"direction\": \"" + myDirection + "\",\"speed\": " + String(anemSpeed) + "}}";
...
void PostVideo(String filename) {
  ...
  while (retryCount < 3) {
    ...
    if (statusCode == 200) {
      ...
      retryCount = 5;
    } else if (statusCode == -3 && retryCount < 2) {
      Serial.println("Video error, retrying upload...\n");
    ...
    } else if (retryCount > 1 && statusCode == -3) {
      Serial.println("Video upload failed, find better signal or restart device.\n");
      retryCount++;
...
void DebugHandler() {
  ...
  long pastmil = millis(), curmil = millis(), inter = pauseTime;

  Serial.println("Debug Handler:\n0.\tDebug Mode On\n1.\tDebug Mode Off (default)\nContinue in " + String(inter) + " seconds.\n");

  while (curmil - pastmil < (inter * 1000)) {
    mqttClient.loop();
...
void menu() {
  ...
  Serial.println("----------'k' -------------------------- photoelectric alarm on -------");
  Serial.println("----------'j' -------------------------- photoelectric alarm off ------");
  Serial.println("----------'m' --------------------------- ionization alarm on ---------");
  Serial.println("----------'n' --------------------------- ionization alarm off --------");
  Serial.println("----------'f' ------------------------------ fan ----------------------");
  Serial.println("----------'l' ------------------------------ LED ----------------------");
  Serial.println("----------'a' ----------------------------- anemometer ----------------");
  Serial.println("----------'v' ---------------------------- wind vane ------------------");
  Serial.println("----------'9' ----------------- mock test 01: Fire---------------------");
  Serial.println("----------'8' ----------------- mock test 02: Potential Fire-----------");
  Serial.println("----------'7' ----------------- mock test 03: No Fire------------------");
  Serial.println("----------'6' ----------------- mock test 04: Sensor Issues------------");
    ...
    switch (incomingByte) {
      ...
      case 't':
        ...
        while (incomingByte != 'q') {
          TempHumiFan();
          Serial.print("Temp:\t" + String(temp) + " C, " + String((temp * 9.0 / 5.0) + 32.0) + " F\tHumi:\t" + String(humi) + " %\n");
      ...
      case '9':
        Serial.println("--------------Testing Mock Data Fire---------------");
        MockTestFire();
        break;

      case '8':
        Serial.println("--------------Testing Mock Data Potential Fire---------------");
        MockTestPotentialFire();
        break;

      case '7':
        Serial.println("--------------Testing Mock Data No Fire---------------");
        MockTestNoFire();
        break;

      case '6':
        Serial.println("--------------Testing Mock Data Sensor Issues---------------");
        MockTestSendSensorIssueValues();
        break;
...
void GSMcoordinates() {
  ...
  while (elapsedTime < searchTime)  //while our elapsed time is less than 4 minutes
  {
  ...
  if ((gsmLati == 0) && (gsmLongi == 0)) {
    Serial.println("Could not get coordinates from the first iteration");
  }

  if ((gsmLati2 == 0) && (gsmLongi2 == 0)) {
    Serial.println("Second iteration coordinates are 0,0. We will use the first iteration coordinates, if any were returned.");
    GSMkey = 3;
  }
}
    ...
    if (GPSSerial.available() > 0)
      if (gps.encode(GPSSerial.read())) {
        ...
        if ((GPSlongi != 0) && (GPSlati != 0)) {
          ...
          Serial.print(GPSlati, 6);
          Serial.print(", Longitude: ");
          Serial.println(GPSlongi, 6);
          break;
        }
      }
  }
...
void Photo() {
  ...
  for (int i = 0; i < 20; i++) {
    ...
    if (debugFlag == 0) {
      Serial.println("Photoelectric Voltage: " + String(photoSensorValue));
    }

    if (photoSensorValue < thresh) {
      photoSensor = "false";
    } else if (photoSensorValue > thresh) {
      photoSensor = "true";
      break;
    }
...
void Ion() {
  ...
  for (int i = 0; i < 20; i++) {
    ...
    if (debugFlag == 0) {
      Serial.println("Ionization Voltage: " + String(ionSensorValue));
    }

    if (ionSensorValue < thresh) {
      ionSensor = "false";
    }
    if (ionSensorValue > thresh) {
      ionSensor = "true";
...
void MockTestFire() {
  // Assign value to temp and humidity
  ionSensor = "true";
  photoSensor = "true";
  ...
  checkAlertValues();
  photoSensor = "false", ionSensor = "false";
}


void MockTestPotentialFire() {
  ionSensor = "false";
  photoSensor = "false";
  temp = 58;
  humi = 14;
...
void MockTestSendSensorIssueValues() {
  float TemporaryTemperature = temp, TemporaryHumidity = humi, TemporaryAnemSpeed = anemSpeed, TemporaryGSMLat = gsmLati, TemporaryGSMLong = gsmLongi, TemporaryGPSLat = mainLati, TemporaryGPSLong = mainLongi;
  String TemporaryIon = ionSensor, TemporaryPhoto = photoSensor, TemporaryDirection = myDirection, TemporaryVidName = videoName;
  int TemporaryAlarm = photoAlarm;
  ...
  ionSensor = "null", photoSensor = "null";
  // WindDirection and WindSpeed
  myDirection = "ERROR", anemSpeed = -4;
  // Alert and Video
  photoAlarm = 3, videoName = "";
  // GPS
  mainLati = 0, mainLongi = 0;
  // GSM
  gsmLati = 0, gsmLongi = 0;

  // Post Data
  checkAlertValues();

  // Reset Values
  temp = TemporaryTemperature, humi = TemporaryHumidity;
  photoSensor = TemporaryPhoto, ionSensor = TemporaryIon;
  anemSpeed = TemporaryAnemSpeed, photoAlarm = TemporaryAlarm;
  mainLati = TemporaryGPSLat, mainLongi = TemporaryGPSLong;
  gsmLati = TemporaryGSMLat, gsmLongi = TemporaryGSMLong;
}


void checkAlertValues() {
  if (ionSensor == "true" || photoSensor == "true") {
    photoAlarm = 2;
  } else if (temp > 57 && humi < 15) {
    photoAlarm = 1;
